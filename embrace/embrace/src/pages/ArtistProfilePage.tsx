...
const ArtistProfilePage: React.FC<ArtistProfilePageProps> = ({ artistId }) => {
    ...
    if (!artist) {
        return (
            <div className="text-center py-16">
                <h1 className="text-3xl font-bold">Artista Não Encontrado</h1>
    ...
    return (
        <div className="space-y-12">
            ...
            <section>
                <h2 className="text-3xl font-bold mb-6 text-white">Músicas</h2>
                {artistSongs.length > 0 ? (
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                        {artistSongs.map(song => <SongCard key={song.id} song={song} />)}
                    </div>
                ) : (
                    <p className="text-gray-400 bg-gray-800 p-6 rounded-lg">Este artista ainda não enviou nenhuma música.</p>
                )}
            </section>
        </div>
    );
