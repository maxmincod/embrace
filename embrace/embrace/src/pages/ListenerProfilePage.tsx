    ...
    return (
        <div className="space-y-12">
            <div>
                ...
                <p className="text-gray-400 mt-1">Seu espaço pessoal para as músicas que você ama.</p>
            </div>

            <section>
                <h2 className="text-2xl font-bold mb-4 text-white">Músicas Curtidas</h2>
                {likedSongs.length > 0 ? (
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                        {likedSongs.map(song => <SongCard key={song.id} song={song} />)}
                    </div>
                ) : (
                    <p className="text-gray-400">Você ainda não curtiu nenhuma música. Vá explorar!</p>
                )}
            </section>

            <section>
                <h2 className="text-2xl font-bold mb-4 text-white">Artistas Seguidos</h2>
                {followedArtists.length > 0 ? (
                    ...
                    <p className="text-gray-400">Você ainda não está seguindo nenhum artista.</p>
                )}
            </section>
        </div>
    );
