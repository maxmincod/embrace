...
interface AuthContextType {
    ...
    updateMusicianProfile: (artistName: string, bio: string, profilePhoto: string) => boolean;
}
...
export const AuthProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
    ...
    const updateMusicianProfile = (artistName: string, bio: string, profilePhoto: string): boolean => {
        if (!currentUser || currentUser.type !== 'musician') return false;
        ...
        const musicianIndex = MOCK_MUSICIANS.findIndex(m => m.id === musician.id);
        if (musicianIndex !== -1) {
            MOCK_MUSICIANS[musicianIndex] = updatedMusician;
        }
        setCurrentUser(updatedMusician);
        return true;
    };

    return (
        <AuthContext.Provider value={{ currentUser, userType, page, setPage, login, logout, registerMusician, registerListener, toggleLikeSong, isLikedSong, updateMusicianProfile }}>
            {children}
