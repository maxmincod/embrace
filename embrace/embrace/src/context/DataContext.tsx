...
interface DataContextType {
    ...
    updateSongLikes: (songId: string, increment: boolean) => void;
    updateMusician: (musicianId: string, updates: Partial<Musician>) => void;
}
...
export const DataProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
    ...
    const updateSongLikes = (songId: string, increment: boolean) => {
        setSongs(prevSongs =>
            prevSongs.map(song =>
                song.id === songId 
                    ? { ...song, likes: Math.max(0, song.likes + (increment ? 1 : -1)) } 
                    : song
            )
        );
    };

    const updateMusician = (musicianId: string, updates: Partial<Musician>) => {
        setMusicians(prevMusicians =>
            prevMusicians.map(musician =>
                musician.id === musicianId ? { ...musician, ...updates } : musician
            )
        );
        const mockIndex = MOCK_MUSICIANS.findIndex(m => m.id === musicianId);
        if (mockIndex !== -1) {
            MOCK_MUSICIANS[mockIndex] = { ...MOCK_MUSICIANS[mockIndex], ...updates };
        }
    };

    return (
        <DataContext.Provider value={{ songs, musicians, donations, getSongsSorted, getArtistSongs, uploadSong, deleteSong, addDonation, incrementPlayCount, updateSongLikes, updateMusician }}>
            {children}
